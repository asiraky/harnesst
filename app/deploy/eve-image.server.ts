...
// Repositories connected to harnesst are not necessarily harnesst scaffolds. When one omits Eve,
// `npm exec -- eve build` downloads a temporary CLI into npm's cache, but authored modules still
// resolve `import "eve"` from /app and the runtime CMD still expects /app/node_modules/.bin/eve.
// Install this exact release locally as the compatibility fallback so build and runtime use the
// same deterministic framework. A repository-declared Eve always wins.
export const FALLBACK_EVE_VERSION = "0.29.5";
...
# Imported/external agents may omit Eve. npm exec's temporary cache is not visible to authored
# module resolution and does not provide the local binary used by CMD, so ensure both contracts.
RUN if [ ! -x node_modules/.bin/eve ]; then npm install --no-save --package-lock=false --no-audit --no-fund eve@${FALLBACK_EVE_VERSION}; fi
COPY . .
RUN node_modules/.bin/eve build
