...
function provisionalTags(input: {
  ...
  const repository = input.taskId
    ? `eden/publish-${lowercaseLegacyId(input.taskId)}`
...
export async function buildStagedTree(
  input: EveImageBuildInput & {
    ...
    /** Publish task namespacing the provisional tag (`eden/publish-<taskId>:…`). */
    taskId?: string;
  },
): Promise<
...
> {
  ...
  const workDir = await mkdtemp(path.join(tmpdir(), "eden-publish-"));
  try {
    const srcDir = await fetchSource(input, workDir);
 ...
 * `eden/proj-<id8>[-member]:<sha12>` (+`-build` — the suffix convention is load-bearing, see
