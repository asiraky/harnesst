 ...
 * the parent's model. A configuration change in harnesst reaches running agents within the 30-second cache TTL
...
export function rewriteOrgModelImports(source: string, depth: number): string {
  ...
  const replace = (_match: string, quote: string) =>
...
// reach a running agent within the 30-second cache TTL — no code change, no redeploy.
