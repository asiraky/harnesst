 ...
 *   2. harnesst's workspace configuration (`GET <EDEN_MODEL_GATEWAY_URL>/model-config`) — the
...
const MODULE_HEADER = `// eden-model.ts — generated by harnesst. The one place this repo's agents resolve their model.
//
...
async function edenConfiguredModel(agentName: string): Promise<EdenModelConfig> {
  ...
  if (!base || !token) {
    throw new Error(
      'This build has no harnesst model coordinates (EDEN_MODEL_GATEWAY_URL / EDEN_MODEL_GATEWAY_TOKEN) — redeploy the agent from harnesst.',
    );
  }
