...
export const ORG_MODEL_MODULE_FILENAME = "harnesst-model.ts";

/** The module path for a member root, e.g. `agents/bob/agent` → `agents/bob/agent/harnesst-model.ts`. */
export function orgModelModulePath(root: string): string {
  return `${root}/${ORG_MODEL_MODULE_FILENAME}`;
}

/**
 * The import specifier for `harnesst-model.ts` from a file at `depth` directories below the member
 * root: the member's `agent.ts` sits at depth 0 (`./harnesst-model`), a subagent's `agent.ts` at
 * depth 2 (`../../harnesst-model` — `subagents/<name>/agent.ts`).
 */
export function orgModelImportSpecifier(depth = 0): string {
  return depth <= 0 ? "./harnesst-model" : `${"../".repeat(depth)}harnesst-model`;
}
