 ...
 *
 * Normal runtime auth mirrors the chat gateway: the org-scoped
 * `HARNESST_MODEL_GATEWAY_TOKEN` (`edng_`) every deploy injects. Assistant evals instead use a
 * short-lived grant token whose pinned model bypasses workspace inheritance. Nothing but the
 * authenticated scope is trusted from the client, so a runtime `project` is verified to belong
 * to its org before it scopes anything. A workspace with nothing configured gets a 404 with a
 * human-readable message the agent surfaces verbatim; that error is the designed behavior, not
 * a fallback: an unconfigured workspace cannot run any model.
 */
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (evalGrantId) {
    ...
    resolved = await resolveTargetModel(orgId!, {
      agentName: agent,
      subagentPath,
      projectId,
    });
