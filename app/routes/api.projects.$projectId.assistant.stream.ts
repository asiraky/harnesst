...
export async function action(args: ActionFunctionArgs) {
  ...
  if (!ensured.unsupported) {
    prefixParts.push(
      `[Eden] Your working checkout for this conversation is at ${conversationCheckoutPath(session.id)} on branch ${conversationBranch(session.id)}. Do ALL repo edits inside that directory with bash — never in any other checkout or clone — after each turn Eden saves the changes from that directory (only) as pending changes the user reviews and publishes.`,
    );
  }
