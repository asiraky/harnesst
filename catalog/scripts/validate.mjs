...
function validateManifest(where, m) {
  ...
  if (typeof m.subagentCompatible !== "boolean") {
  ...
  const rootOnlyFile = Array.isArray(m.files)
    ? m.files.find(
  ...
  if (
    ...
    fail(
      where,
      rootOnlyFile
        ? `subagentCompatible must be false because "${rootOnlyFile}" is root-only in eve`
        : `subagentCompatible must be false for ${m.type} templates`,
    );
  }
  ...
  function checkResolvedFiles(startKey) {
    ...
    const visit = (key, stack) => {
      ...
      for (const inc of t.manifest.includes ?? []) {
        const childKey = `${inc.type}/${inc.id}`;
        const child = byKey.get(childKey);
        if (
          t.manifest.subagentCompatible === true &&
          child?.manifest.subagentCompatible !== true
        ) {
          fail(
            where,
            `subagent-compatible template includes root-only ${childKey}`,
          );
          return false;
        }
        if (!visit(childKey, nextStack)) return false;
      }
