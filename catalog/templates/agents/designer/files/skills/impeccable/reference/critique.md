...
2. **Confirm the target slugs cleanly**:
   ```bash
   node $HOME/.agents/skills/impeccable/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
eve child-agent gate (overrides the default above):
- The root receives the built-in `agent` tool and the user's critique request authorizes the two assessment calls this workflow requires. Do not ask again.
- Emit A and B as two `agent` calls in the same response so eve runs them concurrently.
- Each message is self-contained: include `/workspace/home`, target, live URL, reference paths, product context, assessment letter, and output contract. Children start with fresh history but share the root sandbox and tools.
- If a child call fails, run only that assessment inline and lead with `⚠️ DEGRADED: single-context (child-agent call failed: <exact error>)`.
...
CLI scan:
```bash
node $HOME/.agents/skills/impeccable/scripts/detect.mjs --json [target]
```
...
4. If mutation is available, start `node $HOME/.agents/skills/impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"max_score":<n>,"na_heuristics":"<comma-separated numbers, or empty>","p0_count":<n>,"p1_count":<n>}' \
     node $HOME/.agents/skills/impeccable/scripts/critique-storage.mjs write "<resolved target>" <body-file>
   ```
...
4. **Read the trend** for context:
   ```bash
   node $HOME/.agents/skills/impeccable/scripts/critique-storage.mjs trend "<resolved target>" 5
   ```
