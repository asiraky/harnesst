...
// MARKETPLACE-OWNED. This is the whole channel in one file, and a marketplace update REWRITES IT
// IN FULL — anything edited here is gone on the next update. That is deliberate: everything that
// varies per agent is configuration, not code. Wake rules live on the Deployment tab (projected in
// as `HARNESST_CHANNEL_GITHUB_*` env), credentials are agent secrets, and what the agent should DO
// about an event belongs in its `instructions.md`. Nothing in this file should ever need a
// customer's hands on it.
//
 ...
 * which the control plane does not depend on, so a predicate tangled into the channel config
 * below would only ever be exercised in production.
 */
...
// The configured channel. eve discovers this module by path and takes its default export; the
// wake settings above were already read from env at module load, so building the channel here
// costs nothing beyond what eve's own discovery pays.
