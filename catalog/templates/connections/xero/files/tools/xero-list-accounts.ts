...
export default defineTool({
  description:
    "List the Xero organisation's chart of accounts (code, name, type, tax type, status). Use the codes when creating draft bills.",
  ...
  async execute() {
    ...
    const res = await fetch(
      `${base.replace(/\/+$/, "")}/api/capabilities/xero/list_accounts`,
      {
        method: "POST",
        headers: {
